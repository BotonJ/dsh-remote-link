...
**恢复码的自动化供给（同日补）**。手工生成 + 手改 YAML 的门槛违背"轻松知
道怎么做"的产品要求，落地为三层：

1. **`remote_recovery` 聊天工具**——setup 动作生成 24 字节 base64url 码
   （约 144 bit 熵）、自动写入插件自有存储并即刻生效；码在回复中展示
   **一次**并附保存指引。码的明文永不落盘（store 只存 SHA-256 摘要，
   `$DSH_HOME/remote-link/recovery.json`，0600）。再次 setup 即轮换，
   立即作废旧码（含配置文件里的静态码）。
2. **存储优先级**——工具写入的 store 优先，静态 `pairing.recoveryCode`
   仅为声明式后备（runner 模式/无人对话时用）。
3. **启动日志指引**——未启用恢复码时打印一行"说'设置恢复码'即可启用"，
   被动可发现性兜底。

展示一次的安全性论证：会话记录与 cordis.yml 同机同信任域（威胁模型是
"设备丢失"而非"宿主被攻破"），真正的红线是**码不得存放在除配置与用户
离线副本之外的持久位置**——store 摘要化 + 工具回复不落盘即守住此线。
